"""Standalone RunPod serverless smoke test — one image + one I2V clip.

Run this BEFORE wiring RunPod into the main orchestrator. It verifies:
  1. Endpoint auth works
  2. Qwen-Image workflow runs end-to-end
  3. Wan I2V workflow runs end-to-end using the generated image
  4. Output bytes decode cleanly into PNG + MP4 on disk

Usage (PowerShell):
    $env:SHORTS_RUNPOD_ENDPOINT_ID = "abc123xyz"
    $env:SHORTS_RUNPOD_API_KEY     = "rpa_xxxxxxxxxxxx"
    python runpod_smoke_test.py

Optional:
    --image-only    skip the I2V stage
    --prompt "..."  override the test prompt
    --out DIR       output directory (default ./runpod_smoke_out)

Cost estimate at H100 80GB serverless ($0.0008/s):
    Qwen image: ~30s + cold start → ~$0.05
    Wan I2V 5s clip: ~5min + cold start → ~$0.30
    Total: ~$0.35 per smoke test
"""
...
# Make sure the local package is importable when running this script from repo root.
SRC = Path(__file__).resolve().parent / "src"
if SRC.exists():
    sys.path.insert(0, str(SRC))

from shorts_pipeline.config.settings import Settings
from shorts_pipeline.image_worker.comfy import load_workflow_bundle
from shorts_pipeline.runpod_adapter import (
    RunPodComfyClient,
    RunPodWanI2VClient,
    RunPodError,
)
...
def main() -> int:
    p = argparse.ArgumentParser(description="RunPod serverless smoke test")
    ...
    p.add_argument("--out", default="./runpod_smoke_out", help="output directory")
    p.add_argument(
        "--image-workflow",
        default="qwen_image_local",
    ...
    if not os.environ.get("SHORTS_RUNPOD_ENDPOINT_ID"):
        print("ERROR: SHORTS_RUNPOD_ENDPOINT_ID env var not set", file=sys.stderr)
        return 2
    if not os.environ.get("SHORTS_RUNPOD_API_KEY"):
        print("ERROR: SHORTS_RUNPOD_API_KEY env var not set", file=sys.stderr)
        return 2

    # Build a Settings object in runpod mode. We do not depend on the user's
    # .env or any other implicit config — env vars are the only source of truth
    # for the smoke test so failures point at the right thing.
    settings = Settings(comfy_mode="runpod")
    ...
    img_client = RunPodComfyClient(settings)

    img_out = out_dir / "smoke_image.png"
    print(f"[1/2] Generating image via RunPod endpoint {settings.runpod_endpoint_id}...")
    ...
    h100_cost = total_s * 0.0008
    print(f"\nSmoke test PASSED — wall {total_s:.1f}s, est H100 cost ${h100_cost:.3f}")
